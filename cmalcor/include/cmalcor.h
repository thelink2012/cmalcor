...
/// User has insufficient permission to access device or it's locked by another
/// process.
...
/// Failed to tell the device about the success of a flash programming
/// operation.
#define CMALCOR_ERROR_BADFLASHTELL (-12)
/// Failed to read from the device memory.
#define CMALCOR_ERROR_BADFLASHREAD (-13)
/// Couldn't complete operation because there's no LED configuration programmed
/// on the device.
...
///
/// The version is a 32 bit value 0x00AABBCC, where AA is the major version, BB
/// the minor and CC the revision.
///
CMALCOR_API
...
///
/// If `buffer` is null, returns the number of bytes required to store the
/// string (including null terminator). If `buffer` isn't null, copies the
/// message string into it and returns the number of bytes copied.
///
/// Returns 0 either if the buffer is too small or if `error` isn't a error
/// constant.
///
CMALCOR_API
...
///
/// Returns 1 if the CM Storm Alcor mice is connected on the system, 0
/// otherwise.
///
CMALCOR_API
...
///
/// The version is a 16 bit value 0xABCD, where A is the major version, B is the
/// minor, C the revision and D seems to be always 0.
///
/// The `error` variable is set, if not null, to a `CMALCOR_ERROR_*` value to
/// specify the success/failure of the function.
///
CMALCOR_API
int CmAlcor_GetFirmwareVersion(int* error);

///
/// Reads range `begin`-`end` (inclusive) from the memory of the mouse into
/// `output`. Do note that `begin` must be 4 byte aligned and `end` 3 byte
/// aligned (due to it being inclusive).
///
/// This is able to read the flash memory, the SRAM memory, and even the AMBA
/// and PPB memory region.
///
/// The `error` variable is set, if not null, to a `CMALCOR_ERROR_*` value to
/// specify the success/failure of the function. The function additionally
...
/// Do note that `begin` must be 4 byte aligned and `end` 3 byte aligned (due to
/// it being inclusive).
///
/// The `error` variable is set, if not null, to a `CMALCOR_ERROR_*` value to
/// specify the success/failure of the function. The function additionally
...
///
/// Enables the LED configuration currently programmed on memory (most likely
/// previously set with `CmAlcor_SetLED`).
///
/// It's an error to call this without any setting programmed, the function will
/// fail with `CMALCOR_ERROR_NOLEDCONFIG`.
///
/// The `error` variable is set, if not null, to a `CMALCOR_ERROR_*` value to
/// specify the success/failure of the function. The function additionally
/// returns 1 on success and 0 on failure.
...
///
/// Disables the LED configuration currently programmed on memory, going back to
...
///
/// This does not enable the LED configuration, use `CmAlcor_EnableCustomLED`
/// for that.
///
/// The `error` variable is set, if not null, to a `CMALCOR_ERROR_*` value to
/// specify the success/failure of the function. The function additionally
...
/// Any of the output parameters may be a null pointer to not retrieve such
/// setting.
///
...
/// specify the success/failure of the function.
///
/// The function returns 1 on success retrieving the settings, and 0 if no
/// setting is programmed on the device OR on failure. For the **return 0**
/// case, the `error` variable will have `CMALCOR_ERROR_NOERROR` if no setting
/// is programmed on the device.
///
CMALCOR_API
int CmAlcor_GetLED(int* error, int* mode, int* brightness, int* red, int* green,
                   int* blue);

/// Returns 1 if there's any setting programmed on the mouse memory, 0
/// otherwise.
///
/// The `error` variable is set, if not null, to a `CMALCOR_ERROR_*` value to
/// specify the success/failure of the function.
